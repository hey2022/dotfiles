...
#+end_src
*** Copy current line LSP diagnostic message
#+begin_src emacs-lisp
(defun kill-lsp-current-line-diagnostic-message ()
  "Add the current LSP line diagonstic message to the kill ring, prompting the user if there are more than one."
  (interactive)
  (let* ((lsp-diagnostics (lsp-cur-line-diagnostics))
    ...
    (if (length> lsp-messages 1)
        (kill-new (completing-read "Message: " lsp-messages))
      (kill-new (car lsp-messages)))))

(defun get-lsp-messages (lsp-diagnostics)
  "Get the \"message\" values from LSP-DIAGNOSTICS."
